...
def test_get_subject_for_token_wrong_type():
    email = "test@example.net"
    token = security.create_confirmation_token(email)
    with pytest.raises(security.HTTPException) as exc_infos:
        security.get_subject_for_token_type(token, "access")
    assert "Token has incorrect type, expected 'access'" == exc_infos.value.detail
...
async def test_get_user_not_found():
    user = await security.get_user("user@not_found.net")
    assert user is None


@pytest.mark.anyio("asyncio")
async def test_authentification_user(confirmed_user: dict):
...
@pytest.mark.anyio("asyncio")
async def test_get_current_user_wront_type_token(registered_user: dict):
